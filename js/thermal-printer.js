...
async function printSaleThermal(sale) {
  ...
  const shopSub = (window.CFG && CFG.shopSubtitle) || "";
  const credit = window.creditFooter ? creditFooter() : "";
  // PT-210 / Goojprt: 58mm; cut pe bohot paper → skipCut + sirf 2 line feed
  const printer = window.shopPrinter || new ThermalPrinter({
    paperWidth: 58,
    chunkSize: 48,
    chunkDelay: 40,
    feedBeforeCut: 2,
  ...
  printer.setSettings({ paperWidth: 58, feedBeforeCut: 2, skipCut: true });
  const w = printer.charsPerLine;
  const line = "-".repeat(Math.min(w, 32));
  const thick = "=".repeat(Math.min(w, 32));
  ...
  // Sirf doCut (2 lines) — pehle alag feed + cut se double blank nikalta tha
  await printer.doCut();
  toast("Printed");
}
...
async function renderSettings() {
  ...
  if (pConn) pConn.onclick = async () => {
    try {
      ...
      if (!window.shopPrinter) window.shopPrinter = new ThermalPrinter({ paperWidth: 58, feedBeforeCut: 2, skipCut: true, debug: false });
...
async function printDayClose(day, totals) {
  ...
  const printer = window.shopPrinter || new ThermalPrinter({ paperWidth: 58, feedBeforeCut: 2, skipCut: true });
  window.shopPrinter = printer;
  if (!printer.isConnected) {
